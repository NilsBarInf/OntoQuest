...
class UserDefinitions(db.Model):
    ...
    profile_id = db.Column(db.Integer, db.ForeignKey('profile.id'), nullable=False)  # Foreign key reference to the associated Profile
    profile_class = db.Column(db.String(150), db.ForeignKey('definition_scores.class_name'))  # Class name associated with the definition score
    profile_definition = db.Column(db.String(1000), db.ForeignKey('definition_scores.definition'))  # Definition associated with the definition score
    action_type = db.Column(db.String(50))  # Type of action performed (agree, pass, selection of alternative, entered definition)
    revised_definition = db.Column(db.String(1000), db.ForeignKey('definition_scores.definition'))  # Revised definition associated with the definition score
