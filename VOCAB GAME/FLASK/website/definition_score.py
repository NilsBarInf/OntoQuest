...
def calculate_weighted_decrement(definition_score, semantic_score):
    ...
    if definition_score:
        # Apply weighted decrement based on the semantic difference and the existing score
...
def adjust_stability_threshold(total_reviews, current_threshold):
    """
    Dynamically adjust the stability interval boundaries based on the total number of reviews.
    ...
    """
    lower_bound = current_threshold[0] + (total_reviews * 0.005)  # Decrease lower bound with increasing reviews
    upper_bound = current_threshold[1] - (total_reviews * 0.005)  # Increase upper bound with increasing reviews
    ...
    return (lower_bound, upper_bound)  # Return the adjusted stability threshold boundaries as a tuple
def check_threshold_stability(definition):
    ...
    current_stability_threshold = adjust_stability_threshold(total_reviews, stability_threshold) # Adjust the stability threshold based on the total number of reviews
...
def find_closest_def(entered_definition, profile_game):
    ...
    if not other_definitions or other_definitions[0].definition =="No definition available":
        print("--- %s seconds ---" % (time.time() - start_time)) # Print the elapsed time for performance measurement
        # Return None if no other definitions are available or if the first definition is "No definition available"
        return None
