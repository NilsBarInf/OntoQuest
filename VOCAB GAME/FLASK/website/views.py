...
from .models import Profile, DepartmentOntologyAssociation, GameInformation, UserDefinitions, UsedWord
from random import *
...
def ontology_selection():
    ...
    if current_user.is_authenticated:
        ...
        return render_template("ontology-selection.html", user=current_user, ontologies_json=ontologies_json)
...
def construct_graph(random_word, onto):
    ...
    for cls in onto.classes():
        if random_word in cls.is_a:
            G.add_node(cls.name)  # Add the superclass as a node
            G.add_edge(cls.name, random_word.name, label="is_a")  # Add edge for subclass relationship
        if cls in random_word.is_a:
            G.add_node(cls.name)  # Add the subclass as a node
            G.add_edge(random_word.name, cls.name, label="is_a")  # Add edge for superclass relationship
        for prop in onto.properties():
            if cls in prop.domain and random_word in prop.range: # Case where the selected class is the range of a property
                G.add_node(cls.name)  # Add the related class as a node
                G.add_edge(cls.name, random_word.name, label=prop.name)  # Add edge for property relationship
            if random_word in prop.domain and cls in prop.range: # Case where the selected class is the domain of a property
                G.add_node(cls.name)  # Add the related class as a node
                G.add_edge(random_word.name, cls.name, label=prop.name)  # Add edge for property relationship
...
def get_previous_used_classes(profile_id, ontology_path):
    ...
    print('prev', previous_used_classes)
...
def get_ontology_information(profile_game):
    ...
    ontology_path = ontology_selected.ontology_url # Retrieve the path to the selected ontology
...
def summary():
    ...
    user_revised_definition = UserDefinitions.query.filter(UserDefinitions.profile_id == current_user.id, or_(UserDefinitions.action_type == "selected", UserDefinitions.action_type == "entered")).all()
    ...
    return render_template('summary.html', user=current_user, user_validated_definition=user_validated_definition,
                           ...
                           user_passed_definition=user_passed_definition, vocab_score=vocab_score)
